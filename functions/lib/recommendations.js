...
// --- Main Function ---
exports.getRecommendations = (0, https_1.onCall)(async (request) => {
    ...
    try {
        ...
        const scoredCandidates = candidates.map(candidate => {
            ...
            return Object.assign(Object.assign({}, candidate), { score, debug: { reasons: debugReasons, rawScore: score } });
        });
        ...
        throw new https_1.HttpsError('internal', 'Failed to generate recommendations');
    }
});
/**
 * Helper to fetch Social Graph (Friends-of-Friends)
 */
async function getSocialGraphCandidates(currentUid) {
    try {
        ...
        const l2Promises = selectedL1.map(uid => db.collection(`users/${uid}/following`).limit(10).get());
        ...
        if (l2IdArray.length === 0)
            return [];
        const chunks = chunk(l2IdArray, 10);
        const profilePromises = chunks.map(batch => db.collection('users').where(admin.firestore.FieldPath.documentId(), 'in', batch).get());

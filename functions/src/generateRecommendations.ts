...
export const scheduledGenerateRecommendations = functions.pubsub
    ...
    for (const user of users) {
      try {
        ...
        batches[currentBatchIndex].set(
          db.collection('recommendations').doc(user.uid),
          {
            updatedAt: now,
...
function countMutualConnections(user: User, candidate: User): number {

...
// Create Community
export const createCommunity = functions.https.onCall(
  async (data: CreateCommunityData, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'Must be authenticated to create community'
      );
    }
    ...
    try {
      ...
      if (invalidMembers.length > 0) {
        throw new functions.https.HttpsError(
          'invalid-argument',
          `Invalid user IDs: ${invalidMembers.join(', ')}`
        );
      }
      ...
      await db.runTransaction(async (transaction) => {
        ...
        // Create audit log
        transaction.set(
          db.doc(`communityModerationLogs/${communityRef.id}/logs/creation`),
          {
            action: 'create',
            adminId,
            timestamp: now,
...
// Add Member
export const addMemberToCommunity = functions.https.onCall(
  async (data: { communityId: string; uidToAdd: string }, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
    ...
    try {
      ...
      await db.runTransaction(async (transaction) => {
        ...
        // Log action
        transaction.set(
          db.doc(`communityModerationLogs/${communityId}/logs/${Date.now()}`),
          {
            action: 'add_member',
...
// Remove Member
export const removeMemberFromCommunity = functions.https.onCall(
  async (data: { communityId: string; uidToRemove: string }, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'Must be authenticated'
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        if (uidToRemove === communityData.adminId) {
          throw new functions.https.HttpsError(
            'failed-precondition',
            'Cannot remove admin - transfer admin role first'
          );
        }
      ...
      throw new functions.https.HttpsError(
        'internal',
        'Error removing member from community'
      );
    }
  }
);
...
export const changeCommunityIcon = functions.https.onCall(
  async (data: { communityId: string; iconUrl: string }, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'Must be authenticated'
    ...
    try {
      ...
      if (communityDoc.data()!.adminId !== context.auth.uid) {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Only admin can change community icon'
        );
      }
...
export const searchUsersForAdd = functions.https.onCall(
  async (data: { q: string; limit?: number }, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'Must be authenticated'

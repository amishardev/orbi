...
export const getCloudinarySignature = functions.https.onCall(
  async (data: SignatureParams, context) => {
    // Ensure user is authenticated
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'Must be authenticated to get upload signature'
      );
    }
...
export const uploadCommunityIconServer = functions.https.onCall(
  async (data: { base64: string; communityId?: string }, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
    ...
    if (!data.base64) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'Must provide base64 image data'
      );
    }

...
export const generateCommunityInvite = functions.https.onCall(
    async (data: GenerateInviteData, context) => {
        if (!context.auth) {
            throw new functions.https.HttpsError(
                'unauthenticated',
                'Must be authenticated to generate invite'
            );
        }
        ...
        if (!communityId || !inviteType) {
            throw new functions.https.HttpsError(
                'invalid-argument',
                'Missing required fields: communityId and inviteType'
            );
        }
        ...
        if (inviteType !== 'public' && inviteType !== 'private') {
            throw new functions.https.HttpsError(
                'invalid-argument',
                'inviteType must be "public" or "private"'
            );
        }
        ...
        try {
            ...
            if (communityData.adminId !== context.auth.uid) {
                throw new functions.https.HttpsError(
                    'permission-denied',
                    'Only community admin can generate invite links'
                );
            }
...
// ==========================================
// 2. VALIDATE COMMUNITY INVITE
// ==========================================
interface ValidateInviteData {
    inviteCode: string;
}

export const validateCommunityInvite = functions.https.onCall(
    async (data: ValidateInviteData, context) => {
        ...
        try {
            ...
            if (context.auth?.uid) {
                isAlreadyMember = community.members?.includes(context.auth.uid) || false;
            }
...
// ==========================================
// 3. ACCEPT COMMUNITY INVITE
// ==========================================
interface AcceptInviteData {
    inviteCode: string;
}

export const acceptCommunityInvite = functions.https.onCall(
...
// ==========================================
// 4. APPROVE/REJECT JOIN REQUEST (Admin only)
// ==========================================
...
export const processCommunityJoinRequest = functions.https.onCall(
    async (data: ProcessRequestData, context) => {
        if (!context.auth) {
            throw new functions.https.HttpsError(
                'unauthenticated',
        ...
        try {
            await db.runTransaction(async (transaction) => {
                ...
                if (community.adminId !== context.auth!.uid) {
                    throw new functions.https.HttpsError(
                        'permission-denied',
                        'Only the community admin can process join requests'
                    );
                }
...
// ==========================================
// 5. GET PENDING JOIN REQUESTS (Admin only)
// ==========================================
interface GetRequestsData {
    communityId: string;
}

export const getPendingJoinRequests = functions.https.onCall(
    async (data: GetRequestsData, context) => {
        if (!context.auth) {
            throw new functions.https.HttpsError(
                'unauthenticated',
                'Must be authenticated'
...
// ==========================================
// 6. DISABLE INVITE LINK (Admin only)
// ==========================================
interface DisableInviteData {
    inviteCode: string;
}

export const disableCommunityInvite = functions.https.onCall(
    async (data: DisableInviteData, context) => {
        if (!context.auth) {
            throw new functions.https.HttpsError(
                'unauthenticated',
                'Must be authenticated'
        ...
        try {
            ...
            if (communityDoc.data()?.adminId !== context.auth.uid) {
                throw new functions.https.HttpsError(
                    'permission-denied',
                    'Only admin can disable invites'
                );
            }
